-- Obsidian
local map = vim.keymap.set

map("n", "<C-e>", vim.diagnostic.open_float, { desc = "Open floating diagnostic message" })
map("n", "<C-S-m>", vim.diagnostic.setloclist, { desc = "Open diagnostics list" })

map({ "n", "v" }, "<Space>", "<Nop>", { silent = true })

-- My lovely vertical navigation speedups (do not add them to the jumplist)
-- map("n", "<A-j>", "<cmd>keepjumps normal! }<CR>", { silent = true, remap = true })
-- map("v", "<A-j>", "}", { silent = true, remap = true })
-- map("n", "<A-k>", "<cmd>keepjumps normal! {<CR>", { silent = true, remap = true })
-- map("v", "<A-k>", "{", { silent = true, remap = true })

-- Normal mode
map("n", "<A-j>", ":m .+1<CR>==", { noremap = true, silent = true })
map("n", "<A-k>", ":m .-2<CR>==", { noremap = true, silent = true })

-- Visual mode
map("x", "<A-j>", ":move '>+1<CR>gv=gv", { noremap = true, silent = true })
map("x", "<A-k>", ":move '<-2<CR>gv=gv", { noremap = true, silent = true })

-- Move to next occurrence using native search
map("n", "<C->", "*", { silent = true })
map("n", "<C-S-n>", "#", { silent = true })

-- Delete a word by alt+backspace
map({ "i", "c" }, "<A-BS>", "<C-w>", { noremap = true })
map("n", "<A-BS>", "db", { noremap = true })

-- Select whole buffer
map("n", "<C-a>", "ggVG", {})

-- Force reload the buffer
map("n", "<leader>e", "<cmd>e!<cr>", {})

-- Comment out lines
map("n", "<C-_>", "gcc", {})
map("v", "<C-_>", "gc", {})

-- Clear line with cd
map("n", "cd", "0D", {})

-- Switch between buffers
map("n", "H", "<cmd>bprevious<CR>", { silent = true, remap = true })
map("n", "L", "<cmd>bnext<CR>", { silent = true, remap = true })

-- Save in Ctrl+s
map({ "n", "i" }, "<C-s>", function()
  vim.cmd "write"
end, { desc = "Save", silent = true })
vim.api.nvim_create_autocmd("BufWritePre", {
  callback = function(args)
    local is_prisma = vim.bo[args.buf].filetype == "prisma"

    require("conform").format {
      bufnr = args.buf,
      lsp_fallback = true,
      formatters = is_prisma and { "lsp" } or nil,
    }
  end,
})
-- Duplicate lines
map("v", "<C-Up>", "y`>p`<", { silent = true })
map("n", "<C-Up>", "Vy`>p`<", { silent = true })
map("v", "<C-Down>", "y`<kp`>", { silent = true })
map("n", "<C-Down>", "Vy`<p`>", { silent = true })

map("n", "J", "mzJ`z")
map("n", "<C-d>", "<C-d>zz")
map("n", "<C-u>", "<C-u>zz")

-- Swap the p and P to not mess up the clipbard with replaced text
-- but leave the ability to paste the text
map("x", "p", "P", {})
map("x", "P", "p", {})

-- Exit terminal mode with Esc
map("t", "<Esc>", "<C-\\><C-n>", { nowait = true })
-- Exit insert mode with jj when using normal keyboard
map("i", "jj", "<Esc>", { nowait = true })

-- A bunch of useful shortcuts for one-time small actions bound on leader
map("n", "<leader>n", "<cmd>nohlsearch<CR>", { silent = true })
map("n", "<leader><leader>", "zz", { silent = true })
map("n", "<A-s>", [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]])
map("x", "<A-s>", [[y:%s/<C-r>"/<C-r>"/gI<Left><Left><Left>]])
-- Multiword step by step repeated editing
map("n", "<C-n>", "*Ncgn")
map("x", "<C-n>", [[y/\V<C-R>=escape(@", '/\')<CR><CR>Ncgn]])

-- A bunch of remaps for the command lilne mode
map({ "c", "i" }, "<C-a>", "<Home>", { silent = true })
map({ "c", "i" }, "<C-e>", "<End>", { silent = true })
map({ "c", "i" }, "<A-Bs>", "<C-w>", { noremap = true })

--  Pull one line down useful rempaps from the numeric line
map("n", "<C-t>", "%", { remap = true })

map("n", "<S-h>", "<cmd>e#<cr>", { silent = true })

-- Map 'gx' to open the file or URL under cursor
map("n", "gx", function()
  local target = vim.fn.expand "<cfile>"
  vim.fn.system(string.format("open '%s'", target))
end, { silent = false })

map("n", "<C-g>", function()
  local relative_path = vim.fn.expand "%:."
  if relative_path ~= "" then
    vim.fn.setreg("+", relative_path)
    vim.notify(relative_path, 0)
  end
end, { silent = true, noremap = true, desc = "Show and copy relative file path" })

map("n", "<C-q>", function()
  local current_win = vim.api.nvim_get_current_win()
  local current_config = vim.api.nvim_win_get_config(current_win)

  -- If current window is floating, close it
  if current_config.relative ~= "" then
    vim.api.nvim_win_close(current_win, false)
    return
  end

  local normal_windows = {}
  local has_outline = false

  for _, win in ipairs(vim.api.nvim_list_wins()) do
    local config = vim.api.nvim_win_get_config(win)
    if config.relative == "" then -- Non-floating windows
      table.insert(normal_windows, win)
      local bufnr = vim.api.nvim_win_get_buf(win)
      local filetype = vim.bo[bufnr].filetype
      if filetype == "Outline" then
        has_outline = true
      end
    end
  end

  if #normal_windows == 2 and has_outline then
    vim.cmd "qa"
  else
    -- Use the original <C-w>q behavior
    vim.cmd "quit"
  end
end, { silent = true, noremap = true, nowait = true, desc = "Smart quit" })

map("n", "<C-A-Up>", "<Cmd>resize +2<CR>", { silent = true, nowait = true, desc = "Taller" })
map("n", "<C-A-Down>", "<Cmd>resize -2<CR>", { silent = true, nowait = true, desc = "Shorter" })
map("n", "<C-A-Right>", "<Cmd>vertical resize +5<CR>", { silent = true, nowait = true, desc = "Wider" })
map("n", "<C-A-Left>", "<Cmd>vertical resize -5<CR>", { silent = true, nowait = true, desc = "Narrower" })
map("n", "<C-A-o>", "<cmd>wincmd o<CR>", { silent = true, nowait = true, desc = "Quit other windows" })

map("n", "<leader>sv", "<cmd>rightbelow vsplit<cr>", { silent = true, nowait = true, desc = "Split window vertically" })

map(
  "n",
  "<leader>sh",
  "<cmd>rightbelow split<cr>",
  { silent = true, nowait = true, desc = "Split window horizontally" }
)

-- Move between windows
-- map("n", "<C-h>", "<C-w>h", { desc = "Go to left window" })
-- map("n", "<C-j>", "<C-w>j", { desc = "Go to bottom window" })
-- map("n", "<C-k>", "<C-w>k", { desc = "Go to top window" })
-- map("n", "<C-l>", "<C-w>l", { desc = "Go to right window" })

map("n", "<leader>on", "<cmd>ObsidianNew<cr>", { desc = "New note" })
map("n", "<leader>oN", "<cmd>ObsidianNewFromTemplate<cr>", { desc = "New from template" })
map("n", "<leader>oq", "<cmd>ObsidianQuickSwitch<cr>", { desc = "Quick switch" })
map("n", "<leader>ot", "<cmd>ObsidianToday<cr>", { desc = "Today" })
map("n", "<leader>oy", "<cmd>ObsidianYesterday<cr>", { desc = "Yesterday" })
map("n", "<leader>od", "<cmd>ObsidianDailies<cr>", { desc = "Dailies List" })

-- actual note
map("n", "<leader>ob", "<cmd>ObsidianBacklinks<cr>", { desc = "Backlinks" })
map("n", "<leader>or", "<cmd>ObsidianRename<cr>", { desc = "Rename note" })
map("n", "<leader>oo", "<cmd>ObsidianOpen<cr>", { desc = "Open in Obsidian" })
map("n", "<leader>op", "<cmd>ObsidianPasteImg<cr>", { desc = "Paste Image" })
map("n", "<leader>oc", "<cmd>ObsidianTOC<cr>", { desc = "Table of Contents" })

-- links
map("n", "<leader>of", "<cmd>ObsidianFollowLink<cr>", { desc = "Follow link" })
map("n", "gf", "<cmd>ObsidianFollowLink<cr>", { desc = "Follow link" })

map("n", "<cr>", function()
  if require("obsidian").util.cursor_on_markdown_link() then
    return "<cmd>ObsidianFollowLink<cr>"
  end
  return "<cr>"
end, { buffer = true, expr = true })
